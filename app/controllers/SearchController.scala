...
import utils.{PageData, Utils}

import scala.concurrent.Await
import scala.concurrent.duration.Duration

@Singleton
class SearchController@Inject()(genomedao:genomeDao,cc:ControllerComponents) extends AbstractController(cc) {
  ...
  def getKeyWord(input: String, key: String) : String = {
  ...
  def getKeyWordHead(input: String, key: String): String = {
    val index = input.toUpperCase.indexOf(key.toUpperCase)
    val value = input.slice(index, index + key.length)
  ...
  def searchResult(input: String) = {
    ...
    val option = Array("Gene ID", "GO Annotation", "KEGG Annotation", "COG Class Annotation" , "KOG Class Annotation" ,
      "Pfam Annotation" , "Swissprot Annotation", "TrEMBL Annotation","nr Annotation")

    val x = Await.result(genomedao.searchByInput(keys.head.trim), Duration.Inf)
    val r = x.map{y=>
      y.id+y.geneid+y.go+y.kegg + y.cogAnno + y.kogAnno + y.pfam + y.swiss + y.trembl + y.nr
    }


    val flt = x.map(y => ((y.id, y.geneid), Array(y.geneid, y.go, y.kegg,y.cogAnno,y.kogAnno,y.pfam,y.swiss,y.trembl,y.nr)))
